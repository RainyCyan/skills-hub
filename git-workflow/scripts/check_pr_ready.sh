...
# 4. 检查 commit 历史
echo ""
echo "3. 当前分支的 commits (origin/main..HEAD):"
COMMITS=$(git log --oneline origin/main..HEAD 2>&1) || true
if [ -z "$COMMITS" ]; then
    echo "  (无 commits，当前分支与 origin/main 一致)"
...
# 5. 检查改动的文件
echo ""
echo "4. 改动的文件 (origin/main...HEAD):"
FILES=$(git diff --name-only origin/main...HEAD 2>&1) || true
if [ -z "$FILES" ]; then
    echo "  (无文件改动)"
    check "有文件改动" 1
else
...
DIFF_CHECK=$(git diff origin/main...HEAD --check 2>&1) || true
